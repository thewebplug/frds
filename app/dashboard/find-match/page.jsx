"use client";
import { findMatch } from "@/app/apis/general";
...
export default function CreateCase() {
  ...
  const handleFindMatch = async () => {
    ...
    if(response?.data?.statusCode === 200) {
      ...
      // setGone("false")
      // setMatch(res?.data?.body?.data?.Similarity.toFixed(4) || 0)
...
setFilteredForces(temp)
  ...
  const handleModalClose = (e) => {
    if (e.target.classList.contains("find-match__modal")) {
      setModalOpen(false);
    }
  }
  ...
  return (
    <div className="find-match">
      ...
      <form action="" className="find-match__form">
        <h2 className="find-match__form__title">Find Matches</h2>
...
<feBlend mode="normal" in="SourceGraphic" in2="effect1_dropShadow_8554_84493" result="shape"/>
      ...
      {modalOpen && (
        <div className="find-match__modal" onClick={handleModalClose}
        >
          <div className="find-match__modal__inner">
            <div className="find-match__modal__inner__table">
              <div className="find-match__modal__inner__table__header">
                <div>
                  <svg
                    width="12"
                    height="25"
                  ...
                  >
                    ...
                    <path
                      d="M9.5 16.5L2.5 16.5L6 22L9.5 16.5Z"
                      fill="#7E92A2"
                    />
                  </svg>
                  Image
          ...
          >
            ...
            <div>{result?.middle_name}</div>
            <div>{result?.phone_number}</div>
            <div>{result?.source}</div>
            <div>{result?.similarity.toFixed(4)}%</div>
            <div>{result?.city}</div>
